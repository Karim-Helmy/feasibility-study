...
@section('content')
    <form class="form form-horizontal striped-rows form-bordered" method="post" enctype="multipart/form-data"  action="{{ route('super.courses.update',[$edit->id]) }}">
        @csrf
        <div class="form-body">
            <!-- Name -->
            ...
            <!-- Levels -->
            @if(!empty($edit->level()->get()) && ($edit->level()->get()) != null)
            <div class="form-group row">
                <div class="table-responsive">
                    <table class="table mb-0">
                        <thead>
                        <tr>
                            <th>{{ trans('admin.level_name') }}</th>
                            <th>{{ trans('admin.level_number') }}</th>

                        </tr>
                        </thead>
                        <tbody>
                        @foreach($edit->level()->get() as $level)
                        <tr>
                            <td>{{$level->title}}</td>
                            <td>{{$level->ordering}}</td>
                            <td>  <button type="button" class="btn btn-danger btn-round mr-1 mb-1">{{ trans('admin.delete') }}</button></td>
                        </tr>
                        @endforeach
                        </tbody>
                    </table>
